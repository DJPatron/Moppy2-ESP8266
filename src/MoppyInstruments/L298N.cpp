...
int L298N::LAST_BRIDGE = 1;  // This sketch can handle only up to 4 bridges (the max for Arduino Uno)
 ...
 */
unsigned int L298N::MAX_POSITION[] = {0,800,800,800,800};
...
void L298N::setup() {
  ...
  pinMode(5, OUTPUT); // IN1 for bridge 1
  pinMode(4, OUTPUT); // IN2 for bridge 1
  pinMode(0, OUTPUT); // IN3 for bridge 1
  pinMode(2, OUTPUT); // IN4 for bridge 1
...
void L298N::tick()
{
  ...
  if (currentPeriod[1]>0){
    ...
    if (currentTick[1] >= currentPeriod[1]){
      step(1,5,4,0,2); // Bridge 1 is on pin 2,3,4,5 ВНЕС 5 4 0 2
      currentTick[1]=0;
    }
  }
